...
export default function AdminPostsPage() {
  ...
  const fetchPosts = async () => {
    try {
      setFetching(true);
      const response = await axios.get("/api/admin/admin-posts");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("/api/admin/admin-posts", formData);
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const response = await axios.patch(
        `/api/admin/admin-posts/${selectedPost._id}`,
        updateData
      );
  ...
  const handleDelete = async (postId) => {
    ...
    try {
      const response = await axios.delete(`/api/admin/admin-posts/${postId}`);
      if (response.data.success) {
  ...
  const handleVisibilityToggle = async (postId, currentVisibility) => {
    try {
      const response = await axios.patch(`/api/admin/admin-posts/${postId}`, {
