...
export async function PUT(req, { params }) {
  try {
    ...
    const assignedManager = JSON.parse(
      formData.get("assignedManager") || "[]"
    );
    const assignedEmployee = JSON.parse(
      formData.get("assignedEmployee") || "[]"
    );

    task.assignedTeamLead = assignedTeamLead.map((id) => ({
      teamLeadId: id,
    }));

    task.assignedManager = assignedManager.map((id) => ({
      managerId: id,
    }));

    task.assignedEmployee = assignedEmployee.map((id) => ({
      employeeId: id,
    }));
