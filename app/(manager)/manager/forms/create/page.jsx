...
export default function ManagerCreateFormPage() {
    ...
    const TeamLeadItem = ({ tl }) => {
        ...
        const departmentName = tl.depId?.name || tl.departmentName || "No Department";
                ...
                <div className="flex items-center gap-3 flex-1">
                    <Avatar className="h-10 w-10 border-2 border-white shadow-sm group-hover:scale-105 transition-transform">
                        <AvatarFallback className={`font-medium ${isSelected ? 'bg-gradient-to-br from-blue-500 to-blue-600 text-white' : 'bg-gradient-to-br from-gray-100 to-gray-200 text-gray-700'}`}>
                            {tl.firstName?.charAt(0)?.toUpperCase() || tl.email?.charAt(0)?.toUpperCase() || 'U'}
                        </AvatarFallback>
                    </Avatar>
                                                                    ...
                                                                    {teamLeads.map((tl) => {
                                                                        ...
                                                                        return (
                                                                            <option key={teamLeadId} value={teamLeadId} className="text-gray-900">
                                                                                {tl.firstName} {tl.lastName} ({tl.email}) - {departmentName}
                                                                            </option>
                                                                        );
                                                                    })}
                                                                </select>
