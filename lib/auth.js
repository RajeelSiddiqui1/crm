...
export const authOptions = {
  providers: [
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        if (!identifier || !password) throw new Error("Please enter all fields.");
  ...
  callbacks: {
    ...
    async redirect({ url, baseUrl, token }) {
      ...
      if (token.role === "Admin") return "/dashboard/adminhome";
      if (token.role === "Manager") return "/dashboard/managerhome";
      if (token.role === "TeamLead") return "/dashboard/teamleadhome";
      if (token.role === "Employee") return "/dashboard/employeehome";
